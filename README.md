...
*Setup cell B:* `# game.csv data setup` -> loads `games.csv` (game-level, with stop reason)
...
| 4.b | CDF, player defection only | `heatmap_inf_cdf_d.png` |
...
- **All stopping events** -> counts both player defections and computer terminations toward the cumulative total
- **Player defection only** -> counts only rounds where the player chose to defect; computer terminations are excluded from the numerator but the total number of games remains the denominator
...
- **Infinite** (Section 3, 4, & 5): `data/NEWprocessed.rds`, `data/game.csv`
